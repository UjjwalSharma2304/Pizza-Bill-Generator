...
The system must utilize Python dictionaries to store inventory and pricing:
...
Size Selection: The user must be prompted to select a pizza size. The system must validate this input; if the size is invalid, it must prompt again.
...
Case Insensitivity: The system must handle inputs gracefully regardless of case (e.g., "Small", "small", "SMALL" should all work).
...
* &nbsp;The final total must be calculated dynamically as items are added.



#### **4. Output Generation**

Upon completion of the order, the system must display an \*\*Itemized Receipt\*\* containing:
